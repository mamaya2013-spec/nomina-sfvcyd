...
import styles from "./becario-detail.module.css";
...
export default function BecarioDetailClient({ id }: DetailProps) {
  ...
  const fetchDetailData = async () => {
    ...
    try {
      if (selectedSemester.bloqueado) {
        ...
        if (snapshot && snapshot.nomina_becarios_snapshot) {
          const list = snapshot.nomina_becarios_snapshot as any[];
          const p = list.find((b: any) => b.id === id);
          if (p) {
            // Join category from snapshot if missing
            if (!p.categorias_becas && p.categoria_beca_id && snapshot.categorias_becas_snapshot) {
              const cat = (snapshot.categorias_becas_snapshot as any[]).find(
                (c: any) => c.id === p.categoria_beca_id
              );
              if (cat) {
                p.categorias_becas = cat;
              }
            }
            setPerson(p);
          } else {
            toast.error("No se encontró al becario en el snapshot de este semestre cerrado.");
          }
        }
      } else {
        // 1. Fetch live person data
  ...
  if (!person) {
    return (
      <div className={`${styles.notFound} glass-panel`}>
        ...
        <p>No se pudo localizar el becario solicitado en el sistema.</p>
        <button onClick={() => router.push("/dashboard/becarios")} className={styles.backBtn}>
          <ArrowLeft size={16} />
        ...
        <div className={styles.leftColumn}>
          <div className={`${styles.infoCard} glass-panel`}>
            ...
            <div className={styles.infoRow}>
              <CreditCard size={16} />
              <div>
                <span className={styles.infoLabel}>Monto Beca Mensual</span>
                <span className={`${styles.infoVal} mono font-semibold text-emerald`}>
                  ${Number(person.importe_mensual_beca).toLocaleString("es-AR")}
                </span>
              </div>
            </div>
          ...
          {activeTab === "general" && (
            <div className={styles.tabContentGrid}>
              ...
              <div className={`${styles.detailCard} glass-panel`}>
                ...
                <h4 className={styles.sectionDivider}>Cuentas y Cobro</h4>
                <div className={styles.detailGrid}>
                  ...
                  <div className={styles.detailItem}>
                    <span className={styles.detailLabel}>Categoría de Beca</span>
                    <span className={styles.categoryLabel}>
                      Categoría {person.categorias_becas?.numero_categoria || "-"}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className={`${styles.detailCard} glass-panel`} style={{ gridColumn: "span 2" }}>
                ...
                {person.responsables ? (
                  ...
                  <p className="text-secondary" style={{ padding: "10px 0" }}>
                    No hay un responsable asociado a este becario.
                  </p>
                )}
              </div>
            </div>
